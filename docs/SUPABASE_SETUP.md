...
RLS must be enabled on all tables. The publishable key can only INSERT and SELECT on `events` and `metrics_snapshots`. The `users` table additionally allows UPDATE (needed for upsert on return visits).
...
> **No DELETE policies** are created on any table. The publishable key cannot remove existing rows. The `users` table has an UPDATE policy (needed for upsert on return visits). To clean up data, use the Supabase SQL Editor directly (which uses the service role).
